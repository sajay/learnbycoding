...
# First, let's try to think of a recurrence we can use for this problem. We can try some cases:
...
# Every branch calls itself recursively twice, so our runtime is O(2n). 
...
# Solution 2 : With Dynamic prograamming
# All the following code does is repeat the same computation as above 
# Except starting from the base case and building up the solution. 
