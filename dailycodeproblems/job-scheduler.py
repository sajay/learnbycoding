...
# It's tempting to try to use a greedy strategy like pick the largest number (or first), 
# then the 2nd-largest if it's non-adjacent and so on, but these don't work -- 
# there will always be some edge case that breaks it.
...
# However, this solution runs in O(2n) time, since with each call, we're making two further recursive calls. 
...
def largest_non_adjacent(arr):
    ...
    # Solution 2 :
    # We could memoize the results, 
    # or use dynamic programming to store, in an array, 
