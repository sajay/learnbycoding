...
# it should just be the address of prev. And intermediate nodes should have an XOR of next and prev.
...
# Then, given a node, to go to the next node, we have to XOR the current node's both with the 
...
# To implement add, we would need to update current tail's both to be XOR'd by its current 
# both the new node's memory address. 
